...
class Generator(tf.keras.Model):
    def __init__(self, learning_rate=0.001):
        ...
        self.c2 = tf.keras.layers.Conv2DTranspose(filters=16, kernel_size=5, strides=2, padding='same')
        self.b2 = tf.keras.layers.BatchNormalization()
        # Goes to 32x32
        self.c3 = tf.keras.layers.Conv2DTranspose(filters=16, kernel_size=5, strides=2, padding='same')
        self.b3 = tf.keras.layers.BatchNormalization()
        # Goes to 64x64
        self.c4 = tf.keras.layers.Conv2DTranspose(filters=8, kernel_size=5, strides=2, padding='same')
        self.b4 = tf.keras.layers.BatchNormalization()
        # Goes to 128x128
        self.c5 = tf.keras.layers.Conv2DTranspose(filters=4, kernel_size=5, strides=2, padding='same')
        self.b5 = tf.keras.layers.BatchNormalization()
        # Goes to 256x256 (final layers)
        self.c6 = tf.keras.layers.Conv2DTranspose(filters=2, kernel_size=5, strides=2, padding='same')
